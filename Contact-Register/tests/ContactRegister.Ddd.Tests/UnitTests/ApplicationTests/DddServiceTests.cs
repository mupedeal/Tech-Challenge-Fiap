...
namespace ContactRegister.Ddd.Tests.UnitTests.ApplicationTests;
...
public class DddServiceTests
{
	...
	{
		//Arrange
		var baseResult = new List<DddEntity>
		{
			new DddEntity
			{
	...
	public async Task GetDddByCode_ShouldReturnError_WhenValidationFails()
	{
		...
		Assert.Contains(result.Errors, x => x.Code == "Ddd.Validation" && x.Description == $"{nameof(DddEntity.Code)} must be greater than 10 and lesser than 100");
		Assert.Contains(result.Errors, x => x.Code == "Ddd.Validation" && x.Description == $"{nameof(DddEntity.State)} is invalid");
		Assert.Contains(result.Errors, x => x.Code == "Ddd.Validation" && x.Description == $"{nameof(DddEntity.Region)} can't be null");
	}
